...
This value was chosen as it approximates the median number of data points within all segments, which is 127). %So that each instance comprises of 100 data points in four dimensions of speed, acceleration, jerk and bearing rate. This is regarded as the raw point-level feature, and is used in experiments on raw data.
...
\subsubsection{Ensemble of Autoencoders} \label{sec:eae}
...
\section{Experimental Results}
    ...
    \subsection{Experiment 1: 3-weeks/1-week split}
    The set of experiments conducted with the 3-weeks train, 1-week test trips data split.
     Tables \ref{tab:rf1} through \ref{tab:eae1}, depict the confusion matrices for each of the models created with the 3 weeks/1 week train-test data split, showing the classification reports as well as recall and precision scores.

     This subsection presents the results of the experiments conducted using a temporal data split. Tables \ref{tab:rf1} through \ref{tab:eae1} present the confusion matrices and corresponding classification reports, including recall and precision scores, for each of the models trained and evaluated using this temporal data split.
     % RF raw features
    \begin{table}[htbp]
